...
def report_print():
    ...
    try:
        ...
        a=local_today().replace(day=1); b=local_today()
        start=a.isoformat(); end=b.isoformat()

    employees=User.query.filter_by(company_id=c.id,role='employee').order_by(User.name).all()
    selected=User.query.filter_by(id=int(employee_id),company_id=c.id,role='employee').first() if employee_id else None
    users=[selected] if selected else employees
    holidays={h.day for h in Holiday.query.filter(Holiday.company_id==c.id,Holiday.day>=a,Holiday.day<=b).all()}
    normal_daily=int(round((c.weekly_hours or 44)*60/5))
    sheets=[]

    for u in users:
        ps=Punch.query.filter(Punch.employee_id==u.id,
                              Punch.timestamp>=datetime.combine(a,datetime.min.time()),
                              Punch.timestamp<datetime.combine(b+timedelta(days=1),datetime.min.time())).order_by(Punch.timestamp).all()
        by_day=punches_by_day(ps)
        days=[]; total=0; worked_days=0; useful_days=0; absences=0
        d=a
        while d<=b:
            weekday=d.weekday() < 5
            holiday=d in holidays
            useful=weekday and not holiday
            day_ps=by_day.get(d,[])
            # Use the actual punch sequence in the four standard columns.
            times=[p.timestamp.strftime('%H:%M') for p in day_ps[:4]]
            times += ['']*(4-len(times))
            day_total=work_minutes(day_ps)
            if day_total:
                worked_days += 1
                total += day_total
            if useful:
                ...
                if not day_ps:
                    absences += 1
            days.append({
                'date':d, 'times':times, 'total':day_total,
                'normal':normal_daily if useful else 0,
                'holiday':holiday
            })
            d += timedelta(days=1)

        sheets.append({
            'employee':u, 'days':days, 'total':total,
            'normal_total':sum(x['normal'] for x in days),
            'absences':absences, 'attestations':0,
            'worked_days':worked_days, 'useful_days':useful_days
        })

    return render_template('report_print.html', sheets=sheets, start=start, end=end, company=c, generated=local_now())
